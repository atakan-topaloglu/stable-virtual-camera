...
def parse_task(
    ...
    if task == "img2trajvid_s-prob":
        ...
        if task == "img2img":
            ...
            sampled_indices = (
                split_dict["train_ids"] + split_dict["test_ids"]
            )
            all_imgs_path = [all_imgs_path[i] for i in sampled_indices]
            c2ws = c2ws[sampled_indices]
            Ks = Ks[sampled_indices]
    ...
    return (
        ...
        (
            torch.tensor(anchor_c2ws[:, :3]).float() 
            if anchor_c2ws is not None 
            else None
        ),
        (
            torch.tensor(anchor_Ks).float() 
    ...
    **overwrite_options,
    ...
    for i, scene in tqdm(enumerate(scenes), total=len(scenes)):        
        ...
        create_transforms_simple(
            ...
            img_paths=img_paths,
            img_whs=np.array([
                VERSION_DICT["W"], VERSION_DICT["H"]
            ])[None].repeat(num_inputs + num_targets, 0),
            c2ws=c2ws,
            Ks=Ks,
        )
